...
func Live(p *proc, args ...arg) {
	...
	p.execLeft = p.opMeta.CyclesToExec
}
...
func Ld(p *proc, args ...arg) {
	...
	p.storeValToArg(p.opMeta.AllowedArgs[1], args[1], val1)
}
...
func Fork(p *proc, args ...arg) {
	pc := p.loadArgVal(p.opMeta.AllowedArgs[0], args[0])
...
func Lld(p *proc, args ...arg) {
	...
	val1 := p.loadArgVal(p.opMeta.AllowedArgs[0], args[0], true)
	p.carry = false
	if val1 == 0 {
		p.carry = true
	}
